...
You MUST choose one of the listed era names exactly as written.`;
  ...
  try {
    ...
    if (ph) {
      ph.capture({ distinctId, event: 'ai_era_choose_requested', properties: { app: 'curator', model, era_name: currentEra, run_id: runId, input_tokens: inputTokens, output_tokens: outputTokens } });
      await ph.shutdown();
    }
