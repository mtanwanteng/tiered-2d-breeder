...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (ph) {
      ph.capture({ distinctId, event: 'ai_combination_requested', properties: { app: 'curator', model, tier, era_name: eraName, run_id: runId, input_tokens: inputTokens, output_tokens: outputTokens } });
      await ph.shutdown();
    }
    ...
    const ph = getPostHogClient();
    if (ph) {
      ph.capture({ distinctId: 'anonymous', event: 'ai_combination_error', properties: { app: 'curator', model, error_type: String(error) } });
      await ph.shutdown();
    }
