...
Return your evaluation for every goal listed.`;
  ...
  try {
    ...
    if (ph) {
      ph.capture({ distinctId, event: 'ai_era_check_requested', properties: { app: 'curator', model, era_name: eraName, run_id: runId, input_tokens: inputTokens, output_tokens: outputTokens } });
      await ph.shutdown();
    }
